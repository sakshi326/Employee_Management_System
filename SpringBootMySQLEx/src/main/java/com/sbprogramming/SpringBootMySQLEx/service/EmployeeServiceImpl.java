package com.sbprogramming.SpringBootMySQLEx.service;

import com.sbprogramming.SpringBootMySQLEx.model.Employee;
import com.sbprogramming.SpringBootMySQLEx.model.EmployeeDetail;
import com.sbprogramming.SpringBootMySQLEx.model.form.EmployeeForm;
import com.sbprogramming.SpringBootMySQLEx.repository.EmployeeDetailRepository;
import com.sbprogramming.SpringBootMySQLEx.repository.EmployeeRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Sort;
import org.springframework.stereotype.Service;

import java.util.List;
import java.util.Optional;

@Service
public class EmployeeServiceImpl implements EmployeeService {

    @Autowired
    private EmployeeRepository employeeRepository;

    @Autowired
    private EmployeeDetailRepository detailRepository;
    @Override
    public List<Employee> getAllEmployees(){
        return employeeRepository.findAll();
    }
    @Override
    public void saveEmployee(Employee employee){
        this.employeeRepository.save(employee);
    }

    @Override
    public void saveEmployeeDetail(EmployeeDetail employee) {
        detailRepository.save(employee);
    }

    @Override
    public Employee saveEmployee(EmployeeForm form) {
        return null;
    }

    @Override
    public Employee updateEmployee(EmployeeForm employeeForm) {
        return null;
    }

    @Override
    public void saveEmployee(EmployeeDetail employee) {
        detailRepository.save(employee);
    }

    @Override
    public Employee getEmployeeById(Long id) {
        Optional< Employee > optional = employeeRepository.findById(id);
        Employee employee = null;
        if (optional.isPresent()) {
            employee = optional.get();
        } else {
            throw new RuntimeException(" Employee not found for id :: " + id);
        }
        return employee;
    }

    @Override
    public void deleteEmployeeById(Long id) {
        this.employeeRepository.deleteById(id);
    }

    @Override
    public Page<Employee> findPaginated(int pageNo, String sortField, String sortDirection, int pageSize) {
        return null;
    }

    @Override
    public Page< Employee > findPaginated(int pageNo, int pageSize, String sortField, String sortDirection) {
        Sort sort = sortDirection.equalsIgnoreCase(Sort.Direction.ASC.name()) ? Sort.by(sortField).ascending() :
                Sort.by(sortField).descending();

        Pageable pageable = PageRequest.of(pageNo - 1, pageSize, sort);
        return this.employeeRepository.findAll(pageable);
    }

    @Override
    public EmployeeDetail findByEmployee(Employee employee) {
        return detailRepository.findByEmployee(employee);
    }


}
